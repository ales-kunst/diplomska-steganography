 ...
 * @author         Ales Kunst
 */
public class JpegImageEncoderUtils implements JpegImageConstants {
   ...
   /** LOGGER */
   private static final Logger LOGGER = LoggerFactory.getLogger(JpegImageEncoderUtils.class);
   ...
   public static DoubleMatrix calculateExtremeSlowDct(DoubleMatrix aColorComponentMatrix) {
      ...
      for (int u = 0; u < JpegImageEncoderUtils.JPEG_IMAGE_BLOCK_HEIGHT; u++) {
         resultRow = u;

         for (int v = 0; v < JpegImageEncoderUtils.JPEG_IMAGE_BLOCK_WIDTH; v++) {
            resultColumn = v;
            ...
            for (int x = 0; x < JpegImageEncoderUtils.JPEG_IMAGE_BLOCK_HEIGHT; x++) {
               pixelRow = x;

               for (int y = 0; y < JpegImageEncoderUtils.JPEG_IMAGE_BLOCK_WIDTH; y++) {
                  pixelColumn = y;
   ...
   public static byte convertBitsToByte(byte[] aBitsArray) {
      Long resultValue = BitUtils.convertBitsToNumber(aBitsArray, BitNumbering.MSB_FIRST, Type.BYTE);

      return resultValue.byteValue();
   }
    ...
    * @return
    */
   public static byte[] convertBitsToBytes(byte[] aBitsArray) {
      BitBuffer bitBuffer = new BitBuffer(BitNumbering.MSB_FIRST);

      bitBuffer.put(aBitsArray);
   ...
   public static int convertBitsToInt(byte[] aBitsArray) {
      Long resultValue = BitUtils.convertBitsToNumber(aBitsArray, BitNumbering.MSB_FIRST, Type.INT);
   ...
   public static short convertBitsToUnsignedByte(byte[] aBitsArray) {
      Long resultValue = BitUtils.convertBitsToUnsignedNumber(aBitsArray, BitNumbering.MSB_FIRST, Type.BYTE);

      return resultValue.shortValue();
   }
    ...
    * @return
    */
   public static byte[] convertByteListToByteArray(List<Byte> aList) {
      byte[] returnArray = new byte[aList.size()];

      for (int index = 0; index < aList.size(); index++) {
         returnArray[index] = aList.get(index).byteValue();
      }
    ...
    * @return
    */
   public static byte[] convertBytesToBits(byte[] aByteArray) {
      ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream(aByteArray);
      BitWalker            bitWalker            = new BitWalker(byteArrayInputStream, BitNumbering.MSB_FIRST);

      return bitWalker.getBits(0, aByteArray.length * 8);
   }
    ...
    * @return
    */
   public static int[][] getImageMatrix(BufferedImage aSourceImage) {
      int     numberOfBlockRows    = JpegImageUtils.divideWithRoundUp(aSourceImage.getHeight(),
                                                                      JPEG_IMAGE_BLOCK_HEIGHT);
      int     numberOfBlockColumns = JpegImageUtils.divideWithRoundUp(aSourceImage.getWidth(), JPEG_IMAGE_BLOCK_WIDTH);
      int[][] resultMatrix         =
         new int[numberOfBlockRows * JPEG_IMAGE_BLOCK_HEIGHT][numberOfBlockColumns * JPEG_IMAGE_BLOCK_WIDTH];

      for (int row = 0; row < aSourceImage.getHeight(); row++) {
         for (int column = 0; column < aSourceImage.getWidth(); column++) {
            resultMatrix[row][column] = aSourceImage.getRGB(column, row);
         }
      }
   ...
   public static long getJpegImageBlockCoordinates(long aCurrentPoint) {
      ...
      int jpegImageBlockRow = JpegImageUtils.divideWithRoundUp(currentRow, JPEG_IMAGE_BLOCK_HEIGHT);    // current row for jpeg image blocks
      int jpegImageBlockColumn = JpegImageUtils.divideWithRoundUp(currentColumn, JPEG_IMAGE_BLOCK_WIDTH);    // current column for jpeg image blocks
    ...
    * @return
    */
   public static byte getQuantizedCbChromaValue(int aValue) {
      return (byte) ((aValue & 0x00FF00) >> 8);
   }
    ...
    * @return
    */
   public static byte getQuantizedCrChromaValue(int aValue) {
      return (byte) ((aValue & 0x0000FF));
   }
    ...
    * @return
    */
   public static byte getQuantizedYLumaValue(int aValue) {
      return (byte) ((aValue & 0xFF0000) >> 16);
   }
   ...
   protected static int[] getCoordinates(int index) {
      ...
      int coordinate0 = JpegImageUtils.divideWithRoundUp(localIndex, divider0);
      int rest0       = (localIndex % divider0 == 0)
                        ? divider0
                        : (localIndex % divider0);

      // calculation of second coordinate
      int coordinate1 = JpegImageUtils.divideWithRoundUp(rest0, divider1);
      int rest1       = (rest0 % divider1 == 0)
                        ? divider1
                        : (rest0 % divider1);

      // calculation of third coordinate
      int coordinate2 = JpegImageUtils.divideWithRoundUp(rest1, divider2);
