...
public class WriteJpegInJfif implements JpegImageProcessorStep {
   ...
   protected void writeChromaQuantizationTables(OutputStream aOutStream, JpegImageContext aJpegImageContext)
      ...
      for (int index = 0; index < JpegImageUtils.ZIG_ZAG_COORDINATES.length; index++) {
         ...
         byte chromaQuantizationValue = (byte) JpegImageUtils.getChrominanceQuantizationMatrix()[row][column];
   ...
   protected void writeHuffmanEncodedBytes(OutputStream aOutStream, JpegImageContext aJpegImageContext)
      ...
      byte[]                outputByteArray  =
         JpegImageUtils.convertBitsToBytes(aJpegImageContext.getHuffmanEncodedImageBitsArray());
   ...
   protected void writeLumaQuantizationTables(OutputStream aOutStream, JpegImageContext aJpegImageContext)
      ...
      for (int index = 0; index < JpegImageUtils.ZIG_ZAG_COORDINATES.length; index++) {
         ...
         int  row                   = JpegImageUtils.getRow(coordinates);
         int  column                = JpegImageUtils.getColumn(coordinates);
         byte lumaQuantizationValue = (byte) JpegImageUtils.getLuminanceQuantizationMatrix()[row][column];
