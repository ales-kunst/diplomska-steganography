   ...
   protected void decodeBitArrayWithHuffman(byte[] aBitArray) throws IOException {
      ...
      for (int index = 0; index < aBitArray.length; index++) {
         ...
         if (!isHuffmanEncodedNumber && (numberOfNotCompressedBits == 0)) {
            ...
            int    notEncodedNumber     = JpegImageUtils.convertBitsToInt(notEncodedNumberBits);
            int    decodedZrlcNumber    =
               JpegImageUtils.convertFromEncodedValueToCoefficientValue(notEncodedNumberBits.length, notEncodedNumber);
   ...
   protected byte[] encodeBitArrayWithHuffman(byte[] aBitArray) throws IOException {
      ...
      for (int index = 0; index < aBitArray.length; index++) {
         ...
         if ((bitIndex < 7) && (skipNumberOfBits == 0)) {
            ...
            byte byteValue = JpegImageUtils.convertBitsToByte(byteValueBits);
...
//    byte[] b = JpegImageUtils.convertBitsToBytes(resultHuffmanEncodedStream.toByteArray());
//
//    for (byte by : b) {
...
//
//    System.out.println();
//    b = JpegImageUtils.convertByteListToByteArray(huffmanEncodedArray);
//
//    byte[] a = JpegImageUtils.convertBitsToBytes(b);
//
//    for (byte by : a) {
//       System.out.print(String.format("%02X ", by));
//    }
...
//    System.out.println("");
   ...
   protected List<HuffmanElement> getHuffmanTableForElementType(MatrixElementType aElementType) {
      ...
      default :
         JpegImageUtils.throwJpegImageEncoderExeption("Incorrect element type!");
