 ...
 */
public class JpegF5MeDecode implements StegoDecodingAlgorithm, JpegImageConstants {

   /** LOGGER */
   private static final Logger LOGGER = LoggerFactory.getLogger(JpegF5MeDecode.class);
    ...
    * @param aPassword
    */
   public JpegF5MeDecode(byte[] aPassword) {
      password = aPassword;
   }
   ...
   public void decode(DataContext aDataContext, OutputStream aOutputStream) throws Exception {
      ...
      // Gets size of message. It is 64 bits long
      while (headerBitsRead < HEADER_LENGTH_IN_SIZE_IN_BITS) {
      ...
      int          messageSize          = headerBuffer.getInt() - HEADER_LENGTH_IN_SIZE_IN_BITS;
      int          hummingLevelK        = headerBuffer.getInt();
      DoubleMatrix hummingMatrix        = HummingCodesUtil.createHummingMatrix(hummingLevelK);
      int          bitsBufferSize       = hummingMatrix.getColumns();
      int[]        bitsBuffer           = new int[bitsBufferSize];
      int[]        coefficientPositions = new int[hummingMatrix.getColumns()];
      int[]        coefficientValues    = new int[hummingMatrix.getColumns()];
      int          bufferIndex          = 0;
      ...
      while (coeffAvailable && (messageBitsRead < messageSize)) {
         ...
         if ((coeffValue != 0) && !isDc(coeffIndex)) {
            coefficientPositions[bufferIndex] = coeffIndex;
            coefficientValues[bufferIndex]    = coeffValue;
            bitsBuffer[bufferIndex]           = coeffValue & 0x01;
            bufferIndex++;
         }

         if ((bufferIndex == bitsBufferSize) && (messageBitsRead < messageSize)) {
            DoubleMatrix messageBits = HummingCodesUtil.getHxVector(hummingMatrix, bitsBuffer);

            for (double bit : messageBits.toArray()) {
               message.put((int) bit);
               messageBitsRead++;
            }
